    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent align="end" className="w-80 p-0 bg-slate-900 border-white/10 shadow-2xl">
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          ...
          {unreadCount > 0 && (
            <Button variant="ghost" size="sm" onClick={markAllRead} className="h-8 px-2 text-xs text-slate-400 hover:text-white">
              Mark all read
            </Button>
          )}
        </div>
          ...
          {notifications.length > 0 ? (
            <div className="divide-y divide-white/5">
              {notifications.map(notif => (
                <div key={notif.id} className={`p-4 hover:bg-white/5 transition-colors cursor-pointer group ${!notif.read ? 'bg-white/[0.02]' : ''}`}>
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex items-start justify-between gap-2">
                        <p className={`text-sm font-medium leading-none ${!notif.read ? 'text-white' : 'text-slate-300'}`}>
                          {notif.title}
                        </p>
                        <Button 
                          variant="ghost" 
