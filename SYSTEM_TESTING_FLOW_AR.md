...
```mermaid
graph LR
    P1["👨‍💼 1. مدير النظام<br/>(ضبط الحوكمة والعتبات)"] --> P2["👨‍🏫 2. المعلم المشترك<br/>(رصد الغياب والدرجات)"]
    P2 --> P3["🧑‍💼 3. المرشد الأكاديمي<br/>(تنبيه الخطر وSHAP)"]
    P3 --> P4["👨‍🎓 4. الطالب المستهدف<br/>(المساعد والتوجيه)"]
    P4 --> P5["👨‍👩‍👦 5. ولي الأمر<br/>(التقارير والتواصل)"]
    P5 --> P6["🔄 6. إغلاق الحلقة<br/>(التعافي والتوثيق)"]
    ...
    style P4 fill:#f0fdf4,stroke:#22c55e,stroke-width:2px
...
```mermaid
graph TD
    A1["تسجيل دخول المدير<br/>admin@sba.local"] --> A2["التحقق من الهيكلية المزدوجة<br/>جامعة وكليات vs مدرسة ومراحل"]
    A2 --> A3["معايرة عتبات الإنذار المبكر<br/>منخفض &lt;40% • متوسط 40-74% • مرتفع &ge;75%"]
    A3 --> A4["مراجعة نموذج التنبؤ وسجلات التدقيق<br/>XGBoost / LightGBM ROC-AUC"]
    
    style A1 fill:#eff6ff,stroke:#3b82f6,stroke-width:2px
    style A2 fill:#f8fafc,stroke:#94a3b8,stroke-width:1.5px
    style A3 fill:#fee2e2,stroke:#ef4444,stroke-width:2px
    style A4 fill:#f8fafc,stroke:#94a3b8,stroke-width:1.5px
```
...
```mermaid
graph TD
    T1["تسجيل دخول المعلم المشترك<br/>teacher@sba.local"] --> T2["التبديل بين سياق الجامعة والمدرسة<br/>عزل البيانات وتصفية المواد فورياً"]
    T2 --> T3["اختيار مقرر تراكيب البيانات CS201<br/>وتحديد الطالب أحمد الخطيب"]
    T3 --> T4["رصد غياب متكرر غير مبرر 🔴<br/>صفحة الحضور والغياب"]
    T4 --> T5["رصد درجة نصفي متدنية 45/100 📑<br/>صفحة الدرجات والتقييم"]
    T5 --> T6["تسجيل ملاحظة سلوكية وتشتت ⚠️<br/>تفعيل محرك الإنذار التلقائي"]
    
    style T1 fill:#fef3c7,stroke:#f59e0b,stroke-width:2px
    style T2 fill:#f8fafc,stroke:#94a3b8,stroke-width:1.5px
    style T3 fill:#f8fafc,stroke:#94a3b8,stroke-width:1.5px
    style T4 fill:#fee2e2,stroke:#ef4444,stroke-width:1.5px
    style T5 fill:#fee2e2,stroke:#ef4444,stroke-width:1.5px
    style T6 fill:#fee2e2,stroke:#dc2626,stroke-width:2px
```
...
```mermaid
graph TD
    ADV1["تسجيل دخول المرشدة<br/>advisor@sba.local"] --> ADV2["استقبال إشعار الخطر الفوري 🔔<br/>صندوق الوارد وجرس التنبيهات"]
    ADV2 --> ADV3["رصد الخطر في رادار الطلاب<br/>أحمد الخطيب: High Risk &gt; 80%"]
    ADV3 --> ADV4["فتح الملف الشامل 360°<br/>فحص أثر TreeSHAP (الغياب +35% والدرجات +38%)"]
    ADV4 --> ADV5["اعتماد خطة التدخل البيداغوجي ✍️<br/>جلسات تقوية مكثفة + إرشاد أكاديمي"]
    
    style ADV1 fill:#fee2e2,stroke:#ef4444,stroke-width:2px
    style ADV2 fill:#fff1f2,stroke:#f43f5e,stroke-width:1.5px
    style ADV3 fill:#fee2e2,stroke:#dc2626,stroke-width:2px
    style ADV4 fill:#f8fafc,stroke:#94a3b8,stroke-width:1.5px
    style ADV5 fill:#f0fdf4,stroke:#16a34a,stroke-width:2px
```
...
```mermaid
graph TD
    S1["تسجيل دخول الطالب<br/>student@sba.local"] --> S2["معاينة التنبيه التوجيهي الداعم<br/>لوحة التحكم دون أي وصم نفسي"]
    S2 --> S3["فحص المواد ومسار التوقع المستقبلي<br/>السجل الأكاديمي لمقرر CS201"]
    S3 --> S4["استلام تفاصيل خطة التقوية المعتمدة<br/>صفحة التنبيهات"]
    S4 --> S5["استشارة المساعد الذكي 24/7 💬<br/>طلب نصائح مذاكرة واستراتيجيات تعويض"]
    S5 --> S6["مراجعة الخصوصية والموافقة<br/>سجلات الشفافية GDPR / FERPA"]
    
    style S1 fill:#f0fdf4,stroke:#22c55e,stroke-width:2px
    style S2 fill:#f8fafc,stroke:#94a3b8,stroke-width:1.5px
    style S3 fill:#fef3c7,stroke:#f59e0b,stroke-width:1.5px
    style S4 fill:#f8fafc,stroke:#94a3b8,stroke-width:1.5px
    style S5 fill:#eff6ff,stroke:#3b82f6,stroke-width:2px
    style S6 fill:#f8fafc,stroke:#94a3b8,stroke-width:1.5px
```
...
5. **استشارة المساعد البيداغوجي الذكي (AI Chatbot Widget):**
...
6. **فحص إعدادات الخصوصية وموافقة الذكاء الاصطناعي (GDPR / FERPA):**
...
```mermaid
graph TD
    P1["تسجيل دخول ولي الأمر<br/>parent@sba.local"] --> P2["متابعة بطاقة الابن وتنبيهات الخطر<br/>لوحة تحكم ولي الأمر"]
    P2 --> P3["تصدير تقرير الأداء والمخاطر 📥<br/>تحميل تقرير رسمي موثق PDF / CSV"]
    P3 --> P4["التواصل المباشر مع المرشدة ✉️<br/>تأكيد متابعة المنزل والالتزام بالتقوية"]
    
    style P1 fill:#f5f3ff,stroke:#8b5cf6,stroke-width:2px
    style P2 fill:#fee2e2,stroke:#ef4444,stroke-width:1.5px
    style P3 fill:#eff6ff,stroke:#3b82f6,stroke-width:1.5px
...
```mermaid
graph TD
    CL1["المعلم يرصد حضور جلسة التقوية 🟢<br/>حاضر ومتفاعل في صفحة الحضور"] --> CL2["المعلم يرصد درجة الاختبار التعويضي 88/100 🌟<br/>تحسن حقيقي في صفحة الدرجات"]
    CL2 --> CL3["المعلم يوثق نتيجة التدخل (Implemented) 📝<br/>تدوين ملاحظات الأثر وزوال التعثر"]
    CL3 --> CL4["المرشدة تتحقق من اكتمال الخطة ✅<br/>تراجع مؤشر الخطر للمنطقة الآمنة الخضراء"]
    CL4 --> CL5["تحديث تحليلات الدفعة التراكمية 📊<br/>ارتفاع معدل نجاح التدخلات الأكاديمية"]
    
    style CL1 fill:#f0fdf4,stroke:#16a34a,stroke-width:1.5px
    style CL2 fill:#f0fdf4,stroke:#16a34a,stroke-width:2px
    style CL3 fill:#eff6ff,stroke:#3b82f6,stroke-width:2px
    style CL4 fill:#ecfdf5,stroke:#059669,stroke-width:2px
    style CL5 fill:#f5f3ff,stroke:#8b5cf6,stroke-width:2px
```
